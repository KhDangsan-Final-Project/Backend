...
public class MainController {
	...
	public Map<String, Object> insertUser(@RequestBody Map<String, String> param) {
		...
		String grantNoStr = param.get("grantNo");
	    try {
	        if (grantNoStr != null && !grantNoStr.isEmpty()) {
	            dto.setGrantNo(Integer.parseInt(grantNoStr));
	        } else {
	            // grantNo 값이 없을 경우의 처리
	            dto.setGrantNo(1); // 또는 다른 기본값 설정
	        }
	    } catch (NumberFormatException e) {
	        // 숫자 변환 실패 시 예외 처리
	        e.printStackTrace();
	        return Map.of("msg", "잘못된 grantNo 값", "result", false);
	    }
