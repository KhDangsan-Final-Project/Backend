...
public class RoomWebSocketHandler extends TextWebSocketHandler {
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws IOException {
		...
		try {
			...
			if (token != null && !token.isEmpty()) {
				...
				System.out.println("Room User ID: " + userId);
				System.out.println("Room Nickname: " + nickname);
				System.out.println("Room Grant No: " + grantNo);
				if(profile == null) {
					profile = "no profile";
				}
				System.out.println("changed Room profile" + profile);
				System.out.println("Room ID :" + roomId);

				RoomInfoDTO roomInfoDTO = new RoomInfoDTO(roomId, userId, nickname, grantNo, profile);
				System.out.println("준비완료");
				int result = userService.insertRoomInfo(roomInfoDTO);
				if(result > 0) {
	...
	private void handleRoomCreationOrJoining(WebSocketSession session, String roomId, String userId, String nickname,
			Integer grantNo, String profile) throws IOException {
